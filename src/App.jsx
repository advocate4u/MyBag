...
function Header() {
  ...
  return <header className="site-header"><div className="container nav-wrap">
    ...
    <nav className={`nav ${open ? "open" : ""}`} aria-label="Primary navigation">
      ...
      <a className="nav-cta" href={`tel:${companyConfig.phone}`} aria-label={`Call ${companyConfig.name}`}><span className="action-icon" aria-hidden="true">☎</span><span>Call Us</span></a>
    </nav>
  </div></header>;
}
...
function App() {
  ...
  return <>
    ...
    <main>
      ...
      <section className="contact section" id="contact"><div className="container contact-grid"><div className="contact-copy"><p className="eyebrow">Contact us</p><h2>Let’s find the right <em>bag for your need.</em></h2><p>Share your product, quantity or customization requirement with us.</p><a className="phone contact-action" href={`tel:${c.phone}`} aria-label={`Call ${c.name}`}><span className="contact-icon call-icon" aria-hidden="true">☎</span><span><small>Call us</small>{c.phoneDisplay}</span><b aria-hidden="true">→</b></a><a className="email" href={`mailto:${c.email}`}>{c.email}</a><a className="btn btn-whatsapp" href={`https://wa.me/${c.whatsapp.replace(/\D/g,"")}`} target="_blank" rel="noreferrer" aria-label={`WhatsApp ${c.name}`}><span className="contact-icon whatsapp-icon" aria-hidden="true">WA</span><span>WhatsApp Enquiry</span><span aria-hidden="true">→</span></a></div><div className="locations">{c.addresses.map((a,i)=><article className="location" key={a.label}><span className="location-no">0{i+1}</span><h3>{a.label}</h3><p>{a.lines.map((line,j)=><span key={j}>{line}{j<a.lines.length-1&&<br/>}</span>)}</p></article>)}<div className="gst"><span>GST Registration</span><strong>{c.gst}</strong></div></div></div></section>
    </main>
